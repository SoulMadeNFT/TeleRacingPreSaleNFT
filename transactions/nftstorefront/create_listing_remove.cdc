...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    execute {
        //When creating a new order, we need to check whether the following has ever been had
        // an order for the same itemID. if exist, remove it
        let listingIDS = self.storefront.getListingIDs()
        if listingIDS.length != 0{
            for item in listingIDS{
                let listingitem = self.storefront.borrowListing(listingResourceID: item)
                let details = listingitem!.getDetails()
                if(details.nftID == saleItemID){
                    self.storefront.removeListing(listingResourceID: item)
                }
            }
        }
